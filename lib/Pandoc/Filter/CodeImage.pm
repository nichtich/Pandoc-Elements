package Pandoc::Filter::CodeImage;
use strict;
use warnings;
use utf8;
use Encode;
...
sub new {
    ...
    # get options from constructor, from class, or from base class
    my $config = config($class);
    for ( keys %$config ) {
        $opts{$_} //= $class->config->{$_} // $config->{$_};
    }
...
sub match {
    return unless $_[1]->name eq 'CodeBlock';
    return
      ref( $_[0] ) =~ /^Pandoc::Filter::CodeImage::(.+)$/
      ? ( grep { $1 eq $_ } split ' ', $_[1]->class ) > 0
      : 1;
}
...
Pandoc::Filter::CodeImage - transform code blocks into images
...
Either create an instance of this class or a subclass that overrides C<config>:
...
Returns whether this filter can be applied to the given element by checking the
element is a L<CodeBlock|Pandoc::Elements/CodeBlock> and whether the element
has a class with same local name as the subclasses this is called on (for
instance C<foo> for C<Pandoc::Filter::CodeImage::foo>).
